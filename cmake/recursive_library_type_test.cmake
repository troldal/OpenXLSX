...
function(recursive_library_type_test TARGET_NAME)
    set(_has_static FALSE)
    set(_has_shared FALSE)

    # message( STATUS "recursive_library_type_test: TARGET ${TARGET_NAME}" )
    ...
    elseif("${_type}" STREQUAL "INTERFACE_LIBRARY")
        ...
        # message( STATUS "recursive_library_type_test: ${TARGET_NAME} INTERFACE_LINK_LIBRARIES is ${_links}" )
        foreach(_link IN LISTS _links)
            if(TARGET ${_link})
                recursive_library_type_test( ${_link} ) # recurse
                if(${_link}_HAS_STATIC)
                    set(_has_static TRUE)
                    # message( STATUS "   found a static lib in ${_link}" )
                endif()
                if(${_link}_HAS_SHARED)
                    set(_has_shared TRUE)
                    # message( STATUS "   found a shared lib in ${_link}" )
                endif()
            ...
            elseif(IS_ABSOLUTE "${_link}")
                ...
                message( FATAL_ERROR "   absolute: ${_link} - still need to address this" )
            else()
                # _link might be a linker name (-lfoo) or generator expression; handle below
                message( FATAL_ERROR "   linker name? ${_link} - still need to address this" )
            endif()
        endforeach()
    else()
    endif()

    # message( STATUS "   TARGET ${TARGET_NAME} _has_static is ${_has_static}, _has_shared is ${_has_shared}" )
