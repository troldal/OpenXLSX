...
function(recursive_library_type_test TARGET_NAME PREFIX)
    ...
    message( STATUS ">>>>>>>>>>> recursive_library_type_test:${PREFIX} TARGET ${TARGET_NAME}" )

    get_target_property(_type ${TARGET_NAME} TYPE)
    if("${_type}" STREQUAL "STATIC_LIBRARY")
        set(_has_static TRUE)
        message( STATUS ">>>>>>>>>>> recursive_library_type_test:${PREFIX}   ${TARGET_NAME} is STATIC_LIBRARY" )
    elseif("${_type}" STREQUAL "SHARED_LIBRARY")
        set(_has_shared TRUE)
        message( STATUS ">>>>>>>>>>> recursive_library_type_test:${PREFIX}   ${TARGET_NAME} is SHARED_LIBRARY" )
    elseif("${_type}" STREQUAL "INTERFACE_LIBRARY")
        get_target_property(_links ${TARGET_NAME} INTERFACE_LINK_LIBRARIES)
        message( STATUS ">>>>>>>>>>> recursive_library_type_test:${PREFIX}   ${TARGET_NAME} is INTERFACE_LIBRARY, INTERFACE_LINK_LIBRARIES is ${_links}" )
        foreach(_link IN LISTS _links)
            if(TARGET ${_link})
                recursive_library_type_test( ${_link} "  ${PREFIX}" ) # recurse
                if(${_link}_HAS_STATIC)
                    set(_has_static TRUE)
                    message( STATUS ">>>>>>>>>>> recursive_library_type_test:${PREFIX}   recursion found a static lib in ${_link}" )
                endif()
                if(${_link}_HAS_SHARED)
                    set(_has_shared TRUE)
                    message( STATUS ">>>>>>>>>>> recursive_library_type_test:${PREFIX}   recursion found a shared lib in ${_link}" )
                endif()
                # message( STATUS "   ${_link}_HAS_STATIC is ${${_link}_HAS_STATIC}, ${_link}_HAS_SHARED is ${${_link}_HAS_SHARED}" )
    ...
    message( STATUS ">>>>>>>>>>> recursive_library_type_test:${PREFIX} TARGET ${TARGET_NAME} _has_static is ${_has_static}, _has_shared is ${_has_shared}" )
