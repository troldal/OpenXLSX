 ...
 */
Impl::XLWorksheet::XLWorksheet(XLXmlData* xmlData) : XLSheetBase(xmlData)
{
...
Impl::XLCell Impl::XLWorksheet::Cell(const string& ref)
{
    return Cell(XLCellReference(ref));
}

Impl::XLCell Impl::XLWorksheet::Cell(const string& ref) const
{
    return Cell(XLCellReference(ref));
}
 ...
 */
Impl::XLCell Impl::XLWorksheet::Cell(uint32_t rowNumber, uint16_t columnNumber)
{
 ...
 * @details
 */
Impl::XLCellRange Impl::XLWorksheet::Range(const XLCellReference& topLeft, const XLCellReference& bottomRight) const
{
